...
export const updateUserBio = async ({data, token}: UpdateData) => await fetch(`${BASE_URL}/user`, { method: 'PUT', body: JSON.stringify(data), headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` } }).then(res => res.json());
export const completeUserBio = async ({data, token}: UpdateData) => await fetch(`${BASE_URL}/user/complete-profile`, { method: 'POST', body: JSON.stringify(data), headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` } }).then(res => res.json());